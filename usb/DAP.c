...
static uint8_t DAP_Info(uint8_t id, uint8_t* info)
{
    ...
    case DAP_ID_CAPABILITIES:
        info[0] = ((DAP_SWD != 0) ? (1U << 0) : 0U) | ((DAP_JTAG != 0) ? (1U << 1) : 0U) | ((SWO_UART != 0) ? (1U << 2) : 0U) | ((SWO_MANCHESTER != 0) ? (1U << 3) : 0U) |
            /* Atomic Commands  */ (1U << 4) | ((TIMESTAMP_CLOCK != 0U) ? (1U << 5) : 0U) | ((SWO_STREAM != 0U) ? (1U << 6) : 0U);
        length = 1U;
        break;
...
static uint32_t DAP_SWJ_Pins(const uint8_t* request, uint8_t* response)
{
    ...
    wait = (uint32_t)(*(request + 2) << 0) | (uint32_t)(*(request + 3) << 8) | (uint32_t)(*(request + 4) << 16) | (uint32_t)(*(request + 5) << 24);
    ...
    for (; request_count != 0U; request_count--) {
        ...
        if ((request_value & DAP_TRANSFER_RnW) != 0U) {
            ...
            if ((request_value & DAP_TRANSFER_MATCH_VALUE) != 0U) {
                ...
                match_value = (uint32_t)(*(request + 0) << 0) | (uint32_t)(*(request + 1) << 8) | (uint32_t)(*(request + 2) << 16) | (uint32_t)(*(request + 3) << 24);
                request += 4;
            ...
            // Load data
            data = (uint32_t)(*(request + 0) << 0) | (uint32_t)(*(request + 1) << 8) | (uint32_t)(*(request + 2) << 16) | (uint32_t)(*(request + 3) << 24);
            request += 4;
...
static uint32_t DAP_SWD_WriteAbort(const uint8_t* request, uint8_t* response)
{
    ...
    data = (uint32_t)(*(request + 1) << 0) | (uint32_t)(*(request + 2) << 8) | (uint32_t)(*(request + 3) << 16) | (uint32_t)(*(request + 4) << 24);
