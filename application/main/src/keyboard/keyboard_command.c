...
enum command_delay_hander_event {
    COMMAND_BOOTLOADER, //跳转Bootloader
    COMMAND_SLEEP,      //手动休眠
    COMMAND_RESET,      //重置键盘
    COMMAND_BOND,       //清空绑定
    COMMAND_DEVICES     //切换蓝牙设备
};
...
static bool command_common(uint8_t code)
{
    ...
    case KC_Q:
        clear_keyboard();
        devices_id = 0;
        app_timer_start(command_run_timer, APP_TIMER_TICKS(100), (void*)(uint32_t)COMMAND_DEVICES);
        break;
    case KC_W:
        clear_keyboard();
        devices_id = 1;
...
#endif
        app_timer_start(command_run_timer, APP_TIMER_TICKS(1000), (void*)(uint32_t)COMMAND_BOOTLOADER);
        break;
    case KC_P:
        //休眠
        clear_keyboard();
